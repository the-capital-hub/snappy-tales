...
const moul = Moul({ weight: ["400"], subsets: ["latin"], display: "swap" });
const outfit = Outfit({
  weight: ["400", "600"],
  subsets: ["latin"],
  display: "swap",
    ...
    () => {
      ...
      optionsRef.current.forEach((option, index) => {
        ...
        option.addEventListener("mouseenter", () => {
          gsap.to(option, { scale: 1.05, duration: 0.3, ease: "power2.out" });
          setActiveOption(index);
        });
        option.addEventListener("mouseleave", () =>
          gsap.to(option, { scale: 1, duration: 0.3, ease: "power2.out" })
        );
      });
    ...
    <div className="bg-black">
      ...
      >
        ...
        <div className="flex flex-col md:flex-row w-full justify-between items-center gap-12 relative z-20">
          {/* Options */}
          <div className="w-full md:w-auto mt-5 space-y-5">
            {options.map((option, index) => {
              ...
              return (
                <div
                  ...
                  className={`px-6 py-5 text-lg md:text-4xl flex items-center gap-4 text-white relative cursor-pointer ${outfit.className}`}
                  style={{ borderRadius: "10px", backgroundColor: "#1A1A1A99" }}
                >
                  {/* Gradient border */}
                  <div
                    style={{
